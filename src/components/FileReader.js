...
export default class FileReader extends React.Component{
  ...
  handleClick(event) {
    ...
    var that = this;
    setTimeout(function(){
      that.readExcel(that.state.path).then(function (items) {
          that.props.datasChange(items);
  ...
  render() {
    return (
            <div className="panel panel-default panel-info">
                ...
                <LoadingPopup show={this.state.showLoading} text={this.state.text}></LoadingPopup>
            </div>
        );
  }
};
